...
function diferenciaFecha(destino){
    ...
    let dias = Math.ceil(diferencia / (1000 * 3000 * 24 ));
    return dias;

}
function mostrarEventos(){
    const eventoHTML = eventos.map((evento)=> {
        return `
            <div class="evento">
                <div class="dias"> 
                <span class="diasFaltantes">${diferenciaFecha(evento.fecha)}</span>
                <span class="fechaEvento">dias para :</span>
            </div>
    ...
    listaEventos.innerHTML = eventoHTML.join("");
